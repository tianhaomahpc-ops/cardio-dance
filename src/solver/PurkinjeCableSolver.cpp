...
PurkinjeCableSolver::PurkinjeCableSolver(const SimulationConfig& cfg, IonicModel& ionic)
...
void PurkinjeCableSolver::Advance(double dt_pde_ms, double t_mid_ms,
  ...
  if (dbg_env && *dbg_env == '1') {
    ...
    if (dbg_call % 20 == 1) {
      ...
      std::cout << "[stewart-cable@t=" << t_mid_ms << "] Vp=[" << vp_min
                << ", " << vp_max << "] n>0=" << n_above_zero << "/"
                << n_fe_nodes_ << std::endl;
    }
  }
}
