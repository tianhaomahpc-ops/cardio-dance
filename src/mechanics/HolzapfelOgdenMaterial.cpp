...
double HolzapfelOgdenMaterial::StrainEnergy(const mfem::DenseMatrix& F,
  ...
  const double Wiso = (p.a / (2.0 * p.b)) * (std::exp(p.b * (I1 - 3.0)) - 1.0);
...
void HolzapfelOgdenMaterial::PiolaPassive(const mfem::DenseMatrix& F,
  ...
  // Derivatives of W wrt invariants.
  const double dW_dI1 = 0.5 * p.a * std::exp(p.b * (I1 - 3.0));
  const double df = Macaulay(I4f - 1.0);
  const double ds = Macaulay(I4s - 1.0);
  ...
  // P = 2 dW/dI1 * F + 2 dW/dI4f (F f0) o f0 + 2 dW/dI4s (F s0) o s0
  //   + dW/dI8fs ((F f0) o s0 + (F s0) o f0)
  //   + dU/dJ * cof(F)
  // (For volumetric, dJ/dF = cof(F).)
  ...
  // Isotropic: 2 dW/dI1 * F.
  for (int i = 0; i < 3; ++i)
    for (int j = 0; j < 3; ++j) P(i, j) += 2.0 * dW_dI1 * F(i, j);
