...
// Rank-sharded checkpoint I/O for Vm and ionic-model internal state.
...
class CheckpointIO {
 public:
  ...
  void SaveLatest(int step, double t_ms, const mfem::ParGridFunction& vm,
                  const IIonicModel& ionic) const;
  // Load latest checkpoint if present and compatible with current MPI size.
  bool LoadLatest(int& step, double& t_ms, mfem::ParGridFunction& vm,
