...
int main(int argc, char* argv[]) {
  ...
  if (mfem::Mpi::WorldSize() != 1) {
    if (mfem::Mpi::WorldRank() == 0) {
      std::cerr << "split_niederer_x_regions must run with -np 1" << std::endl;
    }
    return 1;
  }
  ...
  try {
    ...
    std::string out_mesh = "benchmarks/niederer/niederer_xsplit.mesh";
    ...
    if (!(x0 < x1 && x1 < x2 && x2 <= x3)) {
      throw std::runtime_error("Require x0 < x1 < x2 <= x3");
    }
    if (attr_atria <= 0 || attr_fibrosis <= 0 || attr_ventricle <= 0) {
      throw std::runtime_error("Output attrs must be positive");
    }
    if (attr_atria == attr_fibrosis || attr_atria == attr_ventricle ||
        attr_fibrosis == attr_ventricle) {
      throw std::runtime_error("Output attrs must be distinct");
    }
    ...
    std::cout << "split_niederer_x_regions wrote: " << out_mesh << "\n"
              << "  heart attrs in: " << heart_attrs_csv << "\n"
              << "  x ranges: atria[" << x0 << "," << x1 << "), fibrosis[" << x1 << "," << x2
              << "), ventricle[" << x2 << "," << x3 << "]\n"
              << "  counts: atria=" << n_atria << ", fibrosis=" << n_fibrosis
              << ", ventricle=" << n_ventricle << ", kept_heart=" << n_kept_heart
              << ", unchanged_other=" << n_other << std::endl;
  } catch (const std::exception& ex) {
    std::cerr << "split_niederer_x_regions failed: " << ex.what() << std::endl;
    return 2;
  }
